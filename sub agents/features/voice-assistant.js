...
export class VoiceAssistantFeature {
  async integrate(outputPath, clinicInfo) {
    ...
    await fs.writeFile(path.join(featuresDir, 'VoiceAssistant.jsx'), `'use client'
...
export function VoiceAssistant() {
  ...
  const handleVoiceCommand = (command) => {
    ...
    if (lowerCommand.includes('book') || lowerCommand.includes('appointment')) {
      ...
      responseText = 'Connecting you to ${clinicInfo.name}. Please hold while I dial the clinic number.'
      setTimeout(() => {
        window.location.href = 'tel:${clinicInfo.phones[0] || ''}'
      }, 2000)
      ...
      responseText = 'We are located at ${clinicInfo.address || clinicInfo.area + ', Delhi'}. Would you like me to open directions in Google Maps?'
    } else if (lowerCommand.includes('price') || lowerCommand.includes('cost')) {
      responseText = 'Our consultation fee is ${clinicInfo.consultationFee || '₹500'}. For specific treatment costs, I recommend scheduling a consultation with our dentist.'
      ...
      responseText = 'For dental emergencies, please call us immediately at ${clinicInfo.phones[0] || 'our clinic number'}. We prioritize emergency cases.'
    } else {
...
`);
    ...
    await fs.writeFile(path.join(libDir, 'voiceHandler.js'), `/**
 * Voice Handler Library
 */
